...
def get_current_pose(link_name):
    ...
    try:
        ...
        pose = get_link_state_service(get_link_state_request) # response = get_link_state_service(get_link_state_request)
        return  pose          #return response.pose
...
def set_link_state(link_name, pose):
    ...
    try:
        ...
        header = Header(set_link_state_request)
        ...
        # Set the target pose
        set_link_state_request.pose = pose.pose
...
if __name__ == '__main__':
    ...
    rate = rospy.Rate(100)  # Set the loop frequency to 100Hz
    while not rospy.is_shutdown():
        ...
        if current_pose:
            ...
            pose = Pose()
            pose = current_pose                         #pose.position = current_pose.position
