...
def talker():
    track = rospy.Publisher('switch', Tracker, queue_size=1) # Publisher for 'switch' topic, with message type 'Tracker'
    ...
    while not rospy.is_shutdown():
        if not sent:  # Check if the message has already been sent
            tracker = Tracker()
            tracker.flag2 = 1
            track.publish(tracker)  # Publish the 'on' message
            sent = True  # Set flag to True after sending the message
            if sent:
                print('message switch on has been sent')
        r.sleep()
